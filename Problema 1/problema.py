...
personas_fuera_del_sistema = maximo_personas * [Persona()]
...
#     print "Tiempo Actual: %0.6f" % (tiempo_actual)
    ...
    if cola_por_llegar.tamano() > 0:
        tiempo_para_evento = proximo_evento(
            ...
            lista_cajeros)
    # En caso de que la cola este vacia, hacemos la llamada de manera distinta
    else:
        tiempo_para_evento = proximo_evento(
            None,
            lista_cajeros)
...
#     print "En la cola de espera hay: %d" % (cola_por_atender.tamano())
    ...
    if cola_por_llegar.tamano() > 0:
        ...
        if cola_por_llegar.primero().tiempo_llegada == 0:
            ...
            # aceptamos
            if cola_por_atender.esta_vacia() and Cajero.existe_cajero_disponible(
                    lista_cajeros):
    ...
    for i in range(maximo_servidores):
        if (lista_cajeros[i].tiempo_servicio > 0
            ...
            # Disminuimos el tiempo de servicio restante para el cliente actual
